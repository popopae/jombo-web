...
import RegisterAddressBody from "../../../app/containers/Pages/RegisterSupplier/RegisterAddressBody";
import { } from 'jest';
import configureStore from 'redux-mock-store';
import { Provider } from 'react-redux';
import thunk from "redux-thunk";
import { AddressRequest, BranchRequest, StatesResponse } from "../../../app/models";
...
const mockStates: RootState.StateListState = [
    {
...
const initialProvince: StatesResponse[] = [{
    id: 1,
    city: 'Phra Nakhon',
    cityLocal: 'พระนคร',
    countryCode: CommonConstant.ThailandCountryCode,
    ...
    subDistrictLocal: 'พระบรมมหาราชวัง',
    postalCode: '10200',
    provinceTaxCode: '10',
},
{
    ...
    subDistrictLocal: 'วังบูรพาภิรมย์',
    postalCode: '10200',
    provinceTaxCode: '10',
}]

const mockSupplierCities: RootState.CitySupplierListState = initialProvince;
const mockDocumentCities: RootState.CityDocumentListState = initialProvince;
const mockBillingCities: RootState.CityBillingListState = initialProvince;
...
describe("<RegisterAddressBody/>", () => {
    ...
    test("Render content correctly with out billing contact", async () => {
        ...
        const store = mockStore({
            ...
            citySupplierList: mockSupplierCities,
            cityDocumentList: mockDocumentCities,
            cityBillingList: mockBillingCities,
        });
        ...
        const wrapper = render(
            <Provider store={store}>
                <RegisterAddressBody
                    ...
                    onChangeStates={testForm.onChangeStates}
        ...
        expect(queryAllByText("Building Name")).toHaveLength(6);
        expect(queryAllByText("Room")).toHaveLength(6);
        expect(queryAllByText("Floor")).toHaveLength(6);
        expect(queryAllByText("Village Name")).toHaveLength(6);
        expect(queryAllByText("House Number")).toHaveLength(6);
        expect(queryAllByText("Moo Number")).toHaveLength(6);
        expect(queryAllByText("Soi")).toHaveLength(6);
        expect(queryAllByText("Street")).toHaveLength(6);
        expect(queryAllByText("Country")).toHaveLength(12);
        expect(queryAllByText("Province/State")).toHaveLength(12);
        expect(queryAllByText("District/City")).toHaveLength(12);
        expect(queryAllByText("Sub District")).toHaveLength(12);
        expect(queryAllByText("Postal Code")).toHaveLength(12);
    });
    ...
    test("Render content correctly", async () => {
        ...
        const stateListState: RootState.StateListState = [];
        ...
        const handleChangeStates = jest.fn(opts => () => { });
        ...
        const wrapper = render(
            <Provider store={store}>
                <RegisterAddressBody
                    ...
                    labelTitleLocal={'mock-title-local'}
                    isRDVerify={mockRequest.supplierRequest.isRDVerify}
    ...
    test("Render content with Corporate Inter(EN) with use OP option", async () => {
        const registerSupplierRequest = {...mockInitialRegisterSupplierRequest };
        registerSupplierRequest.supplierRequest.identityTypeId = 0;
